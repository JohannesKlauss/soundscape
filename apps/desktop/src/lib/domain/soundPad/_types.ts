...
export const SoundPadCreationSchema = z
  .object({
    ...
    playbackType: z
      .union([z.literal('random'), z.literal('round_robin')])
      .default('round_robin'),
    crossfade: z.number().min(0.1).multipleOf(0.1).default(5),
    samples: z
      .array(SoundSampleSchema)
      .nonempty('At least one sample is needed to create a pad'),
  })
  .refine(
    (data) =>
      data.crossfade <= Math.min(...data.samples.map((s) => s.duration)),
    {
...
export type SoundPadEditForm = z.infer<typeof SoundPadCreationSchema> & {
  id: number
}

export type SoundPad = Omit<
  z.infer<typeof SoundPadCreationSchema>,
  'samples'
> & {
  id: number
  sampleIds: number[]
